...
def extract_primary(
    service: DocumentIntelligenceService, path: Path, document_type: str
) -> ReviewData:
    if document_type == "receipt":
        result = service.analyze_receipt(path)
        return project_extraction(map_receipt_result(service.to_dict(result)))
    result = service.analyze_invoice(path)
    return project_extraction(map_invoice_result(service.to_dict(result)))


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare primary Azure extraction with LLM fallback on selected samples."
    )
    ...
    reviewer = AzureOpenAIDocumentReviewer(
        client=client, deployment_name=settings.azure_openai_deployment
    )
